...
Given an array and a permutation, apply the 
permutation to the array. For example, 
given the array ["a", "b", "c"] and the 
...
# Not inplace bad space complexity --> O(N)
def permute_arry(arr,per):
    arr2 = []
    for p in per:
        arr2.append(arr[p])
    return arr2
